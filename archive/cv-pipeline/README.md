...
* **Implementation:** The trained model predicts amodal larval instances from high-resolution overhead camera feeds. Instance masks are reconstructed from raw ROI logits and cropped per-detection. Each worm's central axis is traced via skeletonization, and a distance transform measures body radius along that axis. The median of the top 20% of radius values (the mid-body region) yields a single robust width per worm, avoiding tapered-end and skeleton-branch artifacts. A **Normal Distribution Filter** then removes width outliers beyond $z > 2.5\sigma$, and remaining widths are converted to mm via a fixed camera calibration and classified into developmental stages.
